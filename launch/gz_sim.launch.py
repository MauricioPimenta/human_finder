...
def gz_launch(context, *args, **kwargs):
    ...
    gui_config = PathJoinSubstitution(
        [pkg_human_finder, 'config', 'gui.config'])
    ...
    render_engine_option = ' --render-engine ' + LaunchConfiguration('render_engine').perform(context)


    # Gazebo Simulator
    # Server mode - no GUI
    if (LaunchConfiguration('noGUI').perform(context) == 'true'):
